...
--切换自动换行
map("n", "<leader>w", function() vim.wo.wrap = not vim.wo.wrap end, ops)
...
map("n", "<leader>fW", builtin.grep_string, ops)
...
map("n", "<leader>fB", "<cmd>Telescope bookmarks<CR>", ops)
...
map("n", "<A-i>", lsp.buf.incoming_calls, ops)
map("n", "<A-o>", lsp.buf.outgoing_calls, ops)
map("n", "<A-s>", lsp.buf.signature_help, ops)
map("n", "<A-q>", vim.diagnostic.setloclist, ops)
map("n", "<leader>h", function() lsp.buf.typehierarchy('supertypes') end, ops)
map("n", "<leader>l", function() lsp.buf.typehierarchy('subtypes') end, ops)
map("n", "<leader>fs", lsp.buf.workspace_symbol, ops)
...
-- codeium
map("n", "<leader>cc", "<cmd>Codeium Chat<CR>", ops)
